...
import {  ICreateFolder } from "@/models/activityInterface";
...
type Props = {
  id:string
  refBtnFolder?: Ref<HTMLButtonElement>;
  resetData?:any
};
...
export default function ModalAddFolder({refBtnFolder,id,resetData
}: Props) {
  ...
  const handleSubmit = async (values: ICreateFolder) => {
    const dataImg = fileList.map((dt) => dt.originFileObj as File);
    const data = { ...values, url:dataImg,files:fileList.map((dt)=>{return {name:dt.name}}),work:id };
    const formData = CustomFormData(data);
    try {
      const statusCode = await postdata(() =>
        activityService.createFolder(formData)
      );
      if (statusCode === 201) {
        form.resetFields();
        setIsModalVisible(false);
  ...
  return (
    <>
      <Button
        hidden={refBtnFolder ? true : false}
        icon={<IoAddOutline />}
        onClick={showModal}
        ref={refBtnFolder}
      >
      ...
      >
        <Tabs defaultActiveKey="1" style={{ width: "100%" }} type="line">
          <TabPane tab="Thông tin folder" key={1}>
