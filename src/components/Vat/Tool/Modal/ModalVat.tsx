...
type Props = {
  refBtnVat?: Ref<HTMLButtonElement>;
};

export default function ModalAddVat({ refBtnVat }: Props) {
  const [form] = useForm();
  ...
  const handleSubmit = async (values: Vat) => {
    const res = await postdata(() => systemService.createVat(values));
    if (res === 200 || res === 201) {
