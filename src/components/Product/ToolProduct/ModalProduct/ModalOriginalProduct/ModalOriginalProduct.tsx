...
type Props = {
  refBtnOriginal?: Ref<HTMLButtonElement>;
};
const ModalOriginalProduct = ({ refBtnOriginal }: Props) => {
  ...
  const handleSubmitEdit = async (value: string, id: string, index: number) => {
    try {
      const statusCode = await postdata(() =>
        productService.updateOriginal(id, value)
      );
  ...
  return (
    <>
      <Button
        hidden={refBtnOriginal ? true : false}
