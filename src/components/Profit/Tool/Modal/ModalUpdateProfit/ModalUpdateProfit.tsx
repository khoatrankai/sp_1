...
export default function ModalUpdateProfit({ ID }: Props) {
  const [form] = useForm();
  ...
  const fetchData = async () => {
    const res = await systemService.getProfit(ID);
  ...
  const handleSubmit = async (values: Profit) => {
    const res = await postdata(() => systemService.updateProfit(ID, values));
    if (res === 200 || res === 201) {
