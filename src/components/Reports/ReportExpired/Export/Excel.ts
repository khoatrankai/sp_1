...
export default async function exportToExcel(data: any) {
  try {
    ...
    const worksheet = workbook.addWorksheet("BaoCaoCongNoCanThu");

    worksheet.addRow([]);
    worksheet.mergeCells("B2:H2");
    worksheet.getCell("B2").value = `BÁO CÁO CÔNG NỢ CẦN THU`;
    ...
    worksheet.columns = [
      ...
      { key: "type_project", width: 30 },
      { key: "name_project", width: 40 },
      { key: "code_contract", width: 30 },
    ...
    saveAs(blob, `Báo cáo cần thu hồi công nợ` + ".xlsx");
  } catch (err) {
    console.log(err);
  }
}
