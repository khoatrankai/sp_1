...
import {
  ...
  export default function ModalListPriceQuote({idOpportunity,refBtn}:Props) {
    // const { data: dataVats } = useFetchData<Vat[]>(systemService.getVats);
    const [pageLimit, setPageLimit] = useState<number>(25);
    const [dataSources,setDataSources] = useState<IGetPriceQuote[]>([])
    ...
    const fetchData = async()=>{
        const res = await priceQuoteService.getPriceQuoteByOpportunityID(idOpportunity ?? "")
        if(res.statusCode === 200){
            setDataSources(res.data?.[0]?.datas ?? [])
            setQuantity(res.data?.[0]?.count ?? [])
        }
    }
    useEffect(()=>{
        if(idOpportunity){
    ...
    return (
        ...
        title={`Danh sách có ${quantity} báo giá`}
      >
          <div className="">    
